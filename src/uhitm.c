...
boolean barehanded;
{
	...
	/* it used to say "canseemon" here but that was, because you'd constantly lose turns if you were
...
register struct monst *mtmp;
{
	...
	if (is_safepet(mtmp) && !flags.forcefight) {
	    if (( (!uwep && !(BloodthirstyEffect || u.uprops[BLOODTHIRSTY_EFFECT].extrinsic || have_stormstone())) || (uwep->oartifact != ART_STORMBRINGER && !(BloodthirstyEffect || u.uprops[BLOODTHIRSTY_EFFECT].extrinsic || have_stormstone()) && uwep->oartifact != ART_STROMBRINGER && uwep->oartifact != ART_ALASSEA_TELEMNAR && uwep->oartifact != ART_THRANDUIL_LOSSEHELIN && uwep->oartifact != ART_HEAVY_THUNDERSTORM && uwep->otyp != TECPATL && !(u.twoweap && uswapwep && uswapwep->otyp == TECPATL) && uwep->oartifact != ART_WAND_OF_ORCUS && uwep->oartifact != ART_GENOCIDE && uwep->oartifact != ART_THIRST_FOR_BLOOD && uwep->oartifact != ART_SLAVE_TO_ARMOK && uwep->oartifact != ART_KILLING_EDGE) ) 
		&& (!u.twoweap || !uswapwep 
...
#undef useup_eggs
	...
	if (get_dmg_bonus && tmp > 0) {
		...
		if (uwep && uwep->oartifact == ART_SINSWORD && u.ualign.record < 199) tmp += 1;
		if (uwep && uwep->oartifact == ART_SINSWORD && u.ualign.record < 249) tmp += 1;
		if (uarmf && u.ustuck && !u.uswallow && !sticks(youmonst.data)) tmp += 2;
	...
	if ((!hittxt || (mon->data->msound == MS_COMBAT) || (youmonst.data->msound == MS_COMBAT) || (Role_if(PM_SPACEWARS_FIGHTER) || sanitymessage || Role_if(PM_CAMPERSTRIKER) || Role_if(PM_HUSSY) || FemtrapActiveAriane || Role_if(PM_GANG_SCHOLAR) || Role_if(PM_WALSCHOLAR) || ishaxor || Hallucination) ) &&			/*( thrown => obj exists )*/
	  (!destroyed || (thrown && m_shot.n > 1 && m_shot.o == obj->otyp))) {
		...
		if (thrown) hit(mshot_xname(obj), mon, exclam(tmp));
			...
			case 359: pline("You tell %s that its %s will eventually develop cancer from all the smoking.", mon_nam(mon), mbodypart(mon, LUNG)); break;
			...
			case 424: pline("%s is caught by your spin kick, but keeps fighting.", Monnam(mon)); break;
			...
			case 433: pline("As %s tries to dodge, %s fully steps into a heap, and swears like a tube sparrow. You use the opportunity to slash %s with your sword.", mon_nam(mon), mhe(mon), mhim(mon)); break;
			...
			case 438: pline("%s starts blabbering 'Hey, my pronouns are %s and %s!' You don't want to listen and aim a big fat slap at its %s.", Monnam(mon), mhe(mon), mhim(mon), mbodypart(mon, FACE)); break;
			...
			case 475: pline("%s grumbles incessantly after being kicked by your boots, complaining that you should have cleaned your treaded soles.", Monnam(mon)); break;
...
/* For vamps */
		...
		if (!already_killed) {
			...
			if ((Role_if(PM_SPACEWARS_FIGHTER) || (mon->data->msound == MS_COMBAT) || (youmonst.data->msound == MS_COMBAT) || Role_if(PM_CAMPERSTRIKER) || Role_if(PM_HUSSY) || Role_if(PM_GANG_SCHOLAR) || FemtrapActiveAriane || Role_if(PM_WALSCHOLAR) || ishaxor || Hallucination || (u.usanity > rn2(1000)) ) && !thrown && !rn2(20) && (PlayerHearsSoundEffects) ) {
				...
				case 272: pline("You take down %s and immerse its %s in a heap!", mon_nam(mon), mbodypart(mon, FACE)); break;
				...
				case 423: pline("You realize an attack from behind"); break;
				case 424: pline("You just know that %s is a predator.", mon_nam(mon)); break;
				...
				case 466: pline("%s is poisoned to death.", Monnam(mon)); break;
				...
				case 474: pline("You smash the %s.", mbodypart(mon, HEAD), l_monnam(mon), mbodypart(mon, BLOOD)); break;
				case 475: pline("You knock out %s and dump.", mon_nam(mon), mhis(mon)); break;
				...
				case 592: pline("You smash %s's %s with your wedding ring because you're such a coward.", mon_nam(mon), mbodypart(mon, FACE)); break;
				...
				case 601: pline("You throw a kick, and this time %s actually gets knocked out by it.", mon_nam(mon)); break;
				...
				case 620: pline("You tell %s to die", mon_nam(mon), mhim(mon)); break;
				...
				case 698: pline("Oh whoops, that was supposed to read 'shoot'", mon_nam(mon), mhe(mon)); break;
...
register int roll;
{
	...
	if (could_seduce(&youmonst, mdef, mattk) && rn2(5) ) {
		...
		case 56: pline("%that doesn't harm %s.", which_armor(mdef, W_ARM) ? "Armor " : "Nonexistant armor ", mon_nam(mdef)); break;
		...
		case 309: pline("%s will beat you!", Monnam(mdef)); break;
		...
		case 322: pline("You try to assault %s but slip on a heap.", mon_nam(mdef)); break;
		...
		case 404: pline("You messed up again, and %s is laughing all the time.", mon_nam(mdef)); break;
		...
		case 458: pline("%s tells you to die.", Monnam(mdef)); break;
		...
		case 482: pline("%s presses a log %s.", Monnam(mdef), makeplural(body_part(HAND))); break;
		...
		case 511: pline("You suddenly get a sight of %s's beauty, and find yourself unable to continue fighting.", mon_nam(mdef)); break;
		...
		case 523: pline("You are such a failure.", l_monnam(mdef)); break;
		...
		case 537: pline("You're not wearing."); break;
		...
		case 560: pline("You set fire to %s's house with %s inside, and wait for it to burn down... but then %s starts praying to %s god, and gets teleported out unharmed.", mon_nam(mdef), mhim(mdef), mon_nam(mdef), mhis(mdef)); break;
		...
		case 590: pline("You try to throw a spin kick at %s, but miss (predictably).", mon_nam(mdef)); break;
		...
		case 606: pline("%s uses a crapping bonus of %d.", Monnam(mdef), mdef->crapbonus, body_part(FACE)); break;
		case 607: pline("%s's homing lazer currently has a charge count of %d.", Monnam(mdef), mdef->hominglazer); break;
		...
		case 616: pline("You tell %s that your pronouns are %s and %s. This has no effect on the current battle, obviously, but results in %s mocking you by answering 'For me, your pronouns are 'screw you' and 'get outta here'. Now kindly remove yourself from my sights.'", mon_nam(mdef), uhe(), uhim(), mon_nam(mdef)); break;
		...
		case 623: pline("%s laughs: 'You are running around with %d points of divine protection? Too scared to fight me without that help? Wimp.'", Monnam(mdef), u.ublessed); break;
		...
		case 658: pline("You curse at the fact that the last hitpoint is always the hardest to remove, because that feature is causing you to miss %s way more often than you should!", mon_nam(mdef)); break;
		case 659: pline("%s shouts '%ss out!'", Monnam(mdef), urace.noun); break;
		case 660: pline("%s screams at you.", Monnam(mdef)); break;
		...
		case 734: pline("Did you know that with your %d dexterity, you miss all the time?", ACURR(A_DEX)); break;
...
bladeangerdone:
		...
		return((boolean)passive(mon, sum[i], 0, mattk->aatyp, FALSE));
	     ...
	     /* downside to jabberwock: they have so many melee attacks that are so powerful, they need to be nerfed
	...
	if(/*malive && */!mon->mcan && rn2(3)) {
		...
		break;
	      ...
	      case AD_SSEX:
			...
			if (u.uprops[ITEM_STEALING_EFFECT].extrinsic || ItemStealingEffect || (uarmc && uarmc->oartifact == ART_PERCENTIOEOEPSPERCENTD_THI) || have_stealerstone() || (uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_COPPERED_OFF_FROM_ME) ) {
