...
shk_other_services()
{
	...
	if (Race_if(PM_IRAHA)) {
		verbalize("Get out of here, insufferable scumbag!");
		return;
	}
...
boolean shk_buying;
{
	...
	case ARMOR_CLASS:
	case WEAPON_CLASS:
		/*if (obj->spe > 0) tmp += 10L * (long) obj->spe;*/ /* price ID :P --Amy */
		/* Don't buy activated explosives! */
		if (is_grenade(obj) && obj->oarmed) tmp = 0L;
		break;

	/* used to be that partly used light sources made it cheaper here... PRICE ID :P --Amy */
	...
	struct monst *shkp;
{
	...
	* Amy edit: why only wands? other items can also be either uncursed- or blessed-charged by scrolls,
	* so why the everloving should only wand shops get that?
	*/
