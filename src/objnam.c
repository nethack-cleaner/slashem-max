...
{	/* Hallu */
	...
	switch (obj->oclass) {
		...
		if (typ == FIGURINE && obj->corpsenm > PM_PLAYERMON && obj->corpsenm < NUMMONS)
		    sprintf(eos(buf), " of a%s %s",
			index(vowels,*(mons[obj->corpsenm].mname)) ? "n" : "",
			mons[obj->corpsenm].mname);
		if (typ == ENERGY_SAP && obj->corpsenm > PM_PLAYERMON && obj->corpsenm < NUMMONS)
		    sprintf(eos(buf), " of a%s %s",
			index(vowels,*(mons[obj->corpsenm].mname)) ? "n" : "",
			mons[obj->corpsenm].mname);
		...
		if (obj->enchantment && !(isevilvariant && !(obj->rknown)))
			strcat(buf, flags.simpledescs ? "ench " : "enchanted ");

		if(obj->enchantment && !(isevilvariant && !(obj->rknown)) && obj->known) {
		...
		break;
	    case ROCK_CLASS:
		if (typ == STATUE && obj->corpsenm > PM_PLAYERMON && obj->corpsenm < NUMMONS)
...
register struct obj *obj;
{
	...
	if (obj->oinvisreal) strcat(prefix,"hidden ");
	if (obj->oinvis && !obj->oinvisreal) strcat(prefix, flags.simpledescs ? "invis " : "invisible ");
...
/* there is absolutely no reason to not display this outside of wizard mode! --Amy */
	...
	if (obj->otyp != FAKE_AMULET_OF_YENDOR && obj->otyp != AMULET_OF_YENDOR && !Hallucination) {
...
/*#ifdef MAIL
	...
	if (Hallucination ? !rn2(100) : (obj->greased) ) strcat(prefix, (obj->greased == 3) ? (flags.simpledescs ? "3xgreased " : "thoroughly greased ") : (obj->greased == 2) ? (flags.simpledescs ? "2xgreased " : "strongly greased ") : "greased ");
...
ring:
	...
	case FOOD_CLASS:
		...
		if (obj->otyp == CORPSE && obj->corpsenm > PM_PLAYERMON && obj->corpsenm < NUMMONS && !Hallucination) {
