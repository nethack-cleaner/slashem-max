...
#undef USES_CORPSENM
	...
	case SPBOOK_CLASS:
		...
		if (otmp->spe > 1) otmp->spe = 1; /* trying to exploit the system --Amy */
	    }
...
boolean ordinary;
{
	...
	switch(obj->otyp) {
		...
		case SPE_FINGER_OF_DEATH:
			 ...
			 * Therefore, I'm deciding that magic resistance has to work. --Amy */
...
notamonster:
		...
		if ((bobj = sobj_at(BOULDER, x, y)) != 0) {
			...
			range = 0;
			       ...
			       (t->ttyp == PIT || t->ttyp == SPIKED_PIT || t->ttyp == GIANT_CHASM || t->ttyp == SHAFT_TRAP || t->ttyp == CURRENT_SHAFT ||
...
retry:
	...
	if (!magicalwish && magical_object) {
		verbalize("Har har har, this object is magical and therefore you cannot wish for it.");
