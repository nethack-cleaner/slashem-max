...
xchar x, y;
{
	...
	if ((trap = t_at(x,y)) != 0 &&
			(((trap->ttyp == PIT || trap->ttyp == MANA_PIT || trap->ttyp == ANOXIC_PIT || trap->ttyp == ACID_PIT ||
...
dokick()
{
	...
	/* your legs are not welded to the horse, so you should be able to kick monsters too!!! --Amy */
