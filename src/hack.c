...
trap_of_walls:
   ...
   if (!rn2(iswarper ? 5 : 10) ) {
	...
	if ((!rn2(3) || (!In_sokoban(&u.uz)) ) && isok(randomx, randomy) && ((levl[randomx][randomy].wall_info & W_NONDIGGABLE) == 0) && (levl[randomx][randomy].typ == ROOM || levl[randomx][randomy].typ == CORR || (levl[randomx][randomy].typ == DOOR && levl[randomx][randomy].doormask == D_NODOOR) ) ) {
		...
		if (!rn2(randchance) && (!In_sokoban(&u.uz) || !sobj_at(BOULDER, randomx, randomy) ) ) {
			...
			if (rn2(3)) {
				 ...
				 * the savegame error again. One day I will eradicate newsym() entirely. */
	...
	/* yet another update by Amy: I give up. No idea why the line below is causing the savegame error,
	 * but it is. Apparently, calling this function during saving up the "uwep" or "uswapwep" structures,
...
moverock()
{
	...
	if (isok(rx,ry) && !IS_ROCK(levl[rx][ry].typ) &&
	    ...
	    if (mtmp && !noncorporeal(mtmp->data) &&
		    (!mtmp->mtrapped ||
			 !(ttmp && ((ttmp->ttyp == PIT) || (ttmp->ttyp == MANA_PIT)
		...
		case CURRENT_SHAFT:
			...
			pline("%s%s and %s a %s in the %s!",
			  ...
			  surface(rx, ry));
		    if (PlayerHearsSoundEffects) pline(issoviet ? "Takim obrazom, vy deystvitel'no dumayete, chto vy dostatochno umny, chtoby' golovolomku bloka. Ya ser'yezno somnevayus' v etom." : "Tchueb!");
		    deltrap(ttmp);
		    delobj(otmp);
		    bury_objs(rx, ry);
		    if (cansee(rx,ry)) newsym(rx,ry);
...
walscholardone:
		...
		if ( ( ( (tunnels(youmonst.data) && !needspick(youmonst.data)) || (Race_if(PM_SCURRIER) && !Upolyd) || u.geolysis) ) && flags.eatingwalls && (Flying || Levitation) ) {
	...
	} else if ( ( ( (tunnels(youmonst.data) && !needspick(youmonst.data)) || (Race_if(PM_SCURRIER) && !Upolyd) || u.geolysis) ) && flags.eatingwalls ) {
	    /* Eat the rock. */
	    if (mode == DO_MOVE && still_chewing(x,y)) return FALSE;
	...
	if (closed_door(x,y)) {
	    ...
	    } else if (((tunnels(youmonst.data) && !needspick(youmonst.data)) || (Race_if(PM_SCURRIER) && !Upolyd)) && flags.eatingdoors ) {
		/* Eat the door. */
		if (mode == DO_MOVE && still_chewing(x,y)) return FALSE;
	    } else {
		if (mode == DO_MOVE) {
	...
	if ( (bigmonst(youmonst.data) && !Race_if(PM_TRANSFORMER) ) || FemtrapActivePatricia || (!Upolyd && Race_if(PM_HUMANOID_CENTAUR) ) || (!Upolyd && Race_if(PM_ETHEREALOID) ) || (!Upolyd && Race_if(PM_INCORPOREALOID) ) || (!Upolyd && Race_if(PM_PLAYER_CERBERUS) ) || (!Upolyd && Race_if(PM_CHIROPTERAN) ) || (!Upolyd && Race_if(PM_THUNDERLORD) ) || (!Upolyd && Race_if(PM_PLAYER_JABBERWOCK) ) ) {
	    if (mode == DO_MOVE)
...
domove()
{
	...
	/* In Soviet Russia, stunning is a crippling status effect that will you up. You're not supposed to stand
	...
	if(mtmp) {
		...
		if(mtmp->m_ap_type && !Protection_from_shape_changers
			...
			/* Amy edit: it makes no sense that Stormbringer doesn't auto-attack in this situation!
			 * You bumped into a peaceful monster with a Stormbringer for sake!
	...
	/* up bug where we can reach here even when engulfed and waste a turn... --Amy
...
stillinwater:;
	...
	if (!in_steed_dismounting) { /* if dismounting, we'll check again later */
		...
		boolean pit;
		pit = (trap && (trap->ttyp == PIT || trap->ttyp == SPIKED_PIT || trap->ttyp == GIANT_CHASM || trap->ttyp == MANA_PIT || trap->ttyp == ANOXIC_PIT || trap->ttyp == ACID_PIT));
		if (trap && pit)
...
register boolean newlev;
{
	...
	for (ptr = &u.uentered[0]; *ptr; ptr++) {
		...
		case LEVELFFROOM:
		    ...
		    break;
		case VERMINROOM:
			pline(FunnyHallu ? "Oh great, you entered a room." : "The air in this room is rank with mildew.");
		    if (!issoviet && anymonstinroom(roomno)) wake_nearby();
		    break;
		case CHAOSROOM:
...
dopickup()
{
	...
	if (!can_reach_floor()) {
 	...
 	if (traphere && traphere->tseen) {
		...
		if ((traphere->ttyp == PIT || traphere->ttyp == SPIKED_PIT || traphere->ttyp == GIANT_CHASM || traphere->ttyp == MANA_PIT || traphere->ttyp == ANOXIC_PIT || traphere->ttyp == ACID_PIT) &&
...
boolean tellplayer;
{
	...
	/* Yes I totally know this will call the SJWs and they will whine like mad, but there is an actual reason
...
#ifdef SHOW_DMG                
		if (flags.showdmg && !(DamageMeterBug || u.uprops[DAMAGE_METER_BUG].extrinsic || have_damagemeterstone()) && n > 0) { 
