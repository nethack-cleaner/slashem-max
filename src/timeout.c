 ...
 */
	...
	if (u.larissatimer) {
		...
		if (u.larissatimer == 20) pline("Quick, find a heap to step into!");
		if (u.larissatimer == 10) pline("Hurry up!!! You have to step into a heap!");
		...
		if (u.larissatimer == 0) {
			...
			pline("Well, now things are bad, because you didn't want to step into it.");
		}
	}
	...
	if (u.umoved && FemtrapActiveHenrietta && !rn2(1000)) {
			    ...
			    if (uarmf && uarmf->oartifact == ART_SUCH_A_SIGHER) {
					nomul(-10, "cleaning their shoes", TRUE);
					nomovemsg = "You finally cleaned your soles.";
			    } else {
	...
	if (!rn2(1000) && u.uprops[SENTIENT_HIGH_HEELS].extrinsic) {
			...
			switch (rnd(26)) {
				...
				case 21:
					pline("Ulch - your block-heeled boot hits your body");
	...
	if (u.uprops[TELEPORTING_ITEMS].extrinsic) {
		...
		if (invent) {
			...
			if (!rn2(10000) && (otmpi->otyp != AMULET_OF_ITEM_TELEPORTATION) && !(otmpi->oartifact == ART_SCHWUEU) && !(otmpi->oartifact == ART_HERSAY_PRICE) && (otmpi->otyp != ITEM_TELEPORTING_STONE) && !(objects[otmpi->otyp].oc_material == MT_BONE && rn2(10)) && !stack_too_big(otmpi) ) {
	...
	for(upp = u.uprops; upp < u.uprops+SIZE(u.uprops); upp++)
		...
		case MC_REDUCTION:
		 ...
		 case FEMTRAP_MARIYA:
			...
			pline("You feel that you've got enough distance.");
		 ...
		 case FEMTRAP_INGE:
			...
			pline("You wish you'd still encounter pretty ladies.");
		 ...
		 case FEMTRAP_MARIKE:
			...
			pline("Aww, apparently you no longer want to listen.");
		...
		case FEMTRAP_JENNIFER:
			pline("You've realized that no more are coming.");
			break;
		...
		case FEMTRAP_NICOLE:
			...
			break;
		case FEMTRAP_RITA:
			pline("That bullshit effect where monsters could trigger feminism trap effects and cause you to be affected by them, it has ended. Took way too long. You're really angry at the programmer.");
			break;
		 ...
		 case FEMTRAP_SARAH:
			...
			pline("Apparently the it is depleted.");
