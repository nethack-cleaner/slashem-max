...
stupidsegfault:
	...
	if (mtmp->ispriest || mtmp->isminion) {
	    ...
	    if (StarlitBug || u.uprops[STARLIT_BUG].extrinsic || have_starlitskystone() || (uarmf && uarmf->oartifact == ART_STAR_SOLES) || MonsterGlyphHallu) {
		strcat(buf, "monster");
		return buf;
	    }
	...
	if (mtmp->isshk && !do_hallu) {
	    ...
	    if (StarlitBug || u.uprops[STARLIT_BUG].extrinsic || have_starlitskystone() || (uarmf && uarmf->oartifact == ART_STAR_SOLES) || MonsterGlyphHallu) strcat(buf, "monster");
	...
	if (!do_hallu && mtmp->isegotype && !mtmp->noegodesc && !mtmp->noegodisplay) {
...
static const char * const bogusmons[] = {
	...
	"General Ironside", "General Mohmar Deathstrike", "paladin tank", "emperor overlord", "aurora alpha bomber", "king raptor", "attack outpost", "lag defense tower", "stinger site", "tunnel network", "nuclear missile silo",
...
"Glycocalyx", "Fibrinogen", "Hemophilia", /* hereditary disease */
	...
	"robber", "homie", "quick learner", "supersmart woman", 
...
"Jill", "Trycja", "Kersey", "Sally", "Hannya", "Jynnifyr", "Elke", "Rinka", "Nicoletta", "Betti", "Ina", "Heikipa", "Jora", "Maitine", "Esruth", "Verene", "Lousie", "G-cheater", "Irinella", "Bastian", "Amandina", "Lillie", "Nicyan", "Leodoch", "Mirella", "Queelix", "Fisoa", "Suesska", "Ann", "Nurisha", "Desiree", "Birgit",
