...
/* right : Ye read "Here lies ..."
   ...
   wrong : Ye read "'er lies ..." */
	...
	if (typ == MSGTYP_NOSHOW && !(MommaBugEffect || u.uprops[MOMMA_BUG].extrinsic || (uimplant && uimplant->oartifact == ART_DEINE_MUDDA) || have_mommystone()) && !(uwep && uwep->oartifact == ART_BRAND_BRAND) && !(u.twoweap && uswapwep && uswapwep->oartifact == ART_BRAND_BRAND)) return;
	if (typ == MSGTYP_NOREP && !(MommaBugEffect || u.uprops[MOMMA_BUG].extrinsic || (uimplant && uimplant->oartifact == ART_DEINE_MUDDA) || have_mommystone()) && !(uwep && uwep->oartifact == ART_BRAND_BRAND) && !(u.twoweap && uswapwep && uswapwep->oartifact == ART_BRAND_BRAND) && !strcmp(line, prevmsg)) return;
...
generate_garbage_string()
{
	...
	while (stringlength --> 0) {
		 ...
		 * are meant to be a part of the game, but we don't want to up the player's actual terminal session.
