...
#ifdef SHOW_DMG                
		    if ( (uhurt == 2) && flags.showdmg && !(DamageMeterBug || u.uprops[DAMAGE_METER_BUG].extrinsic || have_damagemeterstone()) && !(uarmc && uarmc->oartifact == ART_CLOAK_OF_THE_CONSORT) ) pline("[-%d -> %d]", damu, (Upolyd ? u.mh : u.uhp) );
