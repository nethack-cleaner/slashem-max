...
dig()
{
	...
	if (digging.down) {
		...
		if (digging.effort <= 50 ||
		    is_lightsaber(uwep) ||
		    ((ttmp = t_at(dpx,dpy)) != 0 &&
			(ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT || ttmp->ttyp == GIANT_CHASM || ttmp->ttyp == MANA_PIT || ttmp->ttyp == ANOXIC_PIT || ttmp->ttyp == ACID_PIT || ttmp->ttyp == SHAFT_TRAP || ttmp->ttyp == CURRENT_SHAFT ||
...
newbossMETALMAFIA:
		 ...
		 * Soviet, but seriously, I'm not gonna bother coding that just for an obscure special mode
...
boolean pit_only;
{
	...
	if ((ttmp && (ttmp->ttyp == MAGIC_PORTAL || ttmp->ttyp == HEEL_TRAP || ttmp->ttyp == LOUDSPEAKER || ttmp->ttyp == KOP_CUBE || ttmp->ttyp == BOSS_SPAWNER || ttmp->ttyp == ARABELLA_SPEAKER || ttmp->ttyp == FART_TRAP || nohole)) ||
	   ...
	   (IS_ROCK(lev->typ) && lev->typ != SDOOR &&
		...
		if (ttmp && (ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT || ttmp->ttyp == MANA_PIT || ttmp->ttyp == ANOXIC_PIT || ttmp->ttyp == ACID_PIT) &&
		    rn2(2)) {
...
struct obj *obj;
{
	...
	if (u.uswallow && attack(u.ustuck)) {
		...
		if (dam <= 0) dam = 1;
		if (obj->otyp == TORPEDO) pline("You deal yourself a deep gash with your torpedo!");
