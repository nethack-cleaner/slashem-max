...
gluttonous()
{
	...
	if(!rn2(4)) {
		...
		if (FunnyHallu) You_feel("totally down!");
...
opentin()		/* called during each move whilst opening a tin */
{
	...
	if(tin.tin->spe < 1) {
	    ...
	    if (tin.tin && tin.tin->oartifact == ART_YASDORIAN_S_PARTLY_EATEN_T) {
			pline("YEEEEEECH! What was in there???");
	...
	register struct obj *otmp;
{
	 ...
	 * Nah, let's make everything completely up, at least if the player has no K-rations. --Amy */
...
eatspecial() /* called after eating non-food */
{
	...
	if (otmp->oclass == GEM_CLASS && otmp->otyp >= DILITHIUM_CRYSTAL && otmp->otyp <= JADE) gain_alla(5); /* why does the game not save the mohs hardness anywhere??? */
	...
	if (otmp->otyp >= ELIF_S_JEWEL && otmp->otyp <= DORA_S_JEWEL) {
		...
		switch (otmp->otyp) {
			...
			case KSENIA_S_JEWEL:
				FemaleTrapKsenia |= FROMOUTSIDE; break;
			case LYDIA_S_JEWEL:
				FemaleTrapLydia |= FROMOUTSIDE; break;
			case CONNY_S_JEWEL:
				FemaleTrapConny |= FROMOUTSIDE; break;
			case KATIA_S_JEWEL:
				FemaleTrapKatia |= FROMOUTSIDE; break;
			case MARIYA_S_JEWEL:
				FemaleTrapMariya |= FROMOUTSIDE; break;
			case ELISE_S_JEWEL:
				FemaleTrapElise |= FROMOUTSIDE; break;
			case RONJA_S_JEWEL:
				FemaleTrapRonja |= FROMOUTSIDE; break;
			case ARIANE_S_JEWEL:
				FemaleTrapAriane |= FROMOUTSIDE; break;
			...
			case JANINA_S_JEWEL:
				FemaleTrapJanina |= FROMOUTSIDE; break;
			case ROSA_S_JEWEL:
				FemaleTrapRosa |= FROMOUTSIDE; break;
...
doeat()		/* generic "eat" command funtion (see cmd.c) */
{
	...
	if (otmp->oclass != FOOD_CLASS && flags.eatingconfirm) {
