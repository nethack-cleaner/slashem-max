    ...
    register xchar worm_tail;	/* mon is actually a worm tail */
{
	...
	if (StarlitBug || u.uprops[STARLIT_BUG].extrinsic || have_starlitskystone() || (uarmf && uarmf->oartifact == ART_STAR_SOLES) ) {
		show_glyph(x,y,cmap_to_glyph(S_grayglyph));
		return;
	}
    ...
    register int x,y;
{
	...
	if (x == u.ux && y == u.uy) {
		...
		if (mon->mtrapped) {
			...
			tt == SPIKED_PIT || tt == GIANT_CHASM || tt == MANA_PIT || tt == ANOXIC_PIT || tt == ACID_PIT || tt == WEB) && (trap && !trap->hiddentrap)) {
			trap->tseen = TRUE;
		    }
		}
    ...
    int x, y, glyph;
{
	...
	if ( ((uarmh && uarmh->oartifact == ART_NO_RMB_VACATION) || (uamul && uamul->oartifact == ART_BUEING) || (uarmh && uarmh->oartifact == ART_WOLF_KING) || (uamul && uamul->oartifact == ART_YOU_HAVE_UGH_MEMORY)) && glyph >= GLYPH_OBJ_OFF && !(glyph >= GLYPH_CMAP_OFF && glyph < (GLYPH_CMAP_OFF + 13) ) && !(glyph >= (GLYPH_CMAP_OFF + 24) && glyph < (GLYPH_CMAP_OFF + 28) ) && !isimportantlocation(x, y) && !(glyph == (GLYPH_CMAP_OFF + 47) ) )
