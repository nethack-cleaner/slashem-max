    ...
    */
	...
	/* still keeping the old monstermaking routine up, but drastically reducing their spawn rate. --Amy */
		 ...
		 * So I need to make a subloop to prevent numbed or frozen players from being completely immobile. */
			...
			} else if (Fast && (!Race_if(PM_FRO) || !rn2(2)) && (!Race_if(PM_MACTHEIST) || !rn2(2)) ) {
			...
			if (is_snow(u.ux, u.uy) && (u.umoved || !rn2(4)) && !Flying && !Levitation) {
				...
				if (canwalkonsnow && ((uarmf && uarmf->otyp == skates4) || (uarmf && uarmf->oartifact == ART_CORINA_S_SNOWY_TREAD)) && !rn2(2)) {
					oldspeed = moveamt;
					moveamt *= 2;
					if (moveamt > (oldspeed + 24)) moveamt = (oldspeed + 24);
				}
			...
			/* unicorns are ultra fast!!! However, they have enough downsides to reign them in. --Amy */
			...
			if (StrongFast && !rn2(10) && (!Race_if(PM_FRO) || !rn2(2)) && (!Race_if(PM_MACTHEIST) || !rn2(2))) moveamt += speedbonus(moveamt / 2, NORMAL_SPEED / 2);
			...
			/* clockwork gets downside: I know this is heavy-handed, but they're just plain too strong
...
&& mtmp3->data != &mons[PM_SHOPKEEPER] && mtmp3->data != &mons[PM_MASTER_SHOPKEEPER] && mtmp3->data != &mons[PM_ELITE_SHOPKEEPER] && mtmp3->data != &mons[PM_BLACK_MARKETEER] && mtmp3->data != &mons[PM_ALIGNED_PRIEST] && mtmp3->data != &mons[PM_MASTER_PRIEST] && mtmp3->data != &mons[PM_ELITE_PRIEST] && mtmp3->data != &mons[PM_HIGH_PRIEST] && mtmp3->data != &mons[PM_DNETHACK_ELDER_PRIEST_TM_] && mtmp3->data != &mons[PM_GUARD] && mtmp3->data != &mons[PM_MASTER_GUARD] && mtmp3->data != &mons[PM_ELITE_GUARD] && mtmp3->data != &mons[PM_CROUPIER] && mtmp3->data != &mons[PM_MASTER_CROUPIER] && mtmp3->data != &mons[PM_ELITE_CROUPIER]
		...
		if (tech_inuse(T_EXTRA_LONG_SQUEAK)) {
			...
			if ( ((mtmp3 = m_at(u.ux + k, u.uy + l)) != 0) && mtmp3->mtame == 0 && mtmp3->mpeaceful == 0) {
				...
				if (humanoid(mtmp3->data) && !resist(mtmp3, RING_CLASS, 0, NOTELL) && !mtmp3->female) {
					...
					if (cansee(mtmp3->mx,mtmp3->my)) pline("%s inhales and feels bad!", Monnam(mtmp3));
			...
			if (practicantterror) {
				pline("%s booms: 'Practicant lass, I collect 500 zorkmids per second.'", noroelaname());
		...
		if (practicantterror && In_voiddungeon(&u.uz) && u.pract_void && (moves > u.pract_voidtimer)) {
			if (!u.pract_voidinitial) {
				pline("%s thunders: 'I called the kops. Get out of the void or the police will give you a life-long sentence in jail!'", noroelaname());
		...
		if (practicantterror && u.practicantcash) { /* Noroela spends your hard-earned cash */
...
newbossJENNY:
		...
		if (uarmc && uarmc->oartifact == ART_INA_S_SORROW && IS_TOILET(levl[u.ux][u.uy].typ) && u.uhs < HUNGRY ) {
			pline("For some reason, you are ill.");
		...
		if (FemtrapActiveYvonne && (multi >= 0) && IS_TOILET(levl[u.ux][u.uy].typ) && u.uhs < HUNGRY ) {
			...
			nomovemsg = "You are done.";
			nomul(-crapduration, "going to the bathroom", TRUE);
...
newbossB:
		...
		if (Conflict && !rn2(500)) { /* downside --Amy :P */

			makerandomtrap(FALSE);

		}

		if (StrongConflict && !rn2(500)) { /* downside --Amy :P */
...
pastds2:
		...
		if (flags.moreforced && !MessagesSuppressed) display_nhwindow(WIN_MESSAGE, TRUE);    /* --More-- */
			    u.utrap += rnd(4);
			    stop_occupation(); /* why doesn't such a thing stop you */
			}
		    }
		}
...
past3:
	...
	if (practicantterror) {
		...
		if (uwep && (uwep->otyp == CIGARETTE || uwep->otyp == ELECTRIC_CIGARETTE || uwep->otyp == CIGAR) && (u.pract_smokingtimer >= 10 && u.pract_smokingtimer <= 95)) {
			u.pract_smokingtimer = 100;
			pline("%s thunders: 'You maggot! I *TOLD* you that smoking is strictly prohibited! Just for that, your fine is doubled now and you have to pay an additional 5000 zorkmids.'", noroelaname());
...
boolean new_game;	/* false => restoring an old game */
{
    ...
    *xtrabuf = '\0';
	...
	if (!new_game && issoviet) {
		...
		/* In Soviet Russia, modders simply assume that everything the Amy does is bad. They do not actually
