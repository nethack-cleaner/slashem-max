...
/*WAC cap off the Monk's ac bonus to -11 */
	...
	if (u.uprops[NAKEDNESS].extrinsic || (uarmg && uarmg->oartifact == ART_SPREAD_YOUR_LEGS_WIDE) || Nakedness || have_nakedstone() ) uac = 10;
...
glibr()
{
	...
	if (u.twoweap && otmp) {
		...
		xfl++;
		if (( (otmp->otyp != LOADSTONE && otmp->otyp != HEALTHSTONE && otmp->otyp != LUCKSTONE && otmp->otyp != MANASTONE && otmp->otyp != SLEEPSTONE && otmp->otyp != LOADBOULDER && otmp->otyp != STARLIGHTSTONE && otmp->otyp != STONE_OF_MAGIC_RESISTANCE ) || !otmp->cursed) && !(otmp->otyp == LUCKSTONE && isevilvariant && !otmp->cursed && !otmp->blessed && Luck < 0))
			dropx(otmp);
	}
	otmp = uwep;
	if (otmp && !welded(otmp)) {
