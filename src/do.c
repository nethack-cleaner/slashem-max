...
const char *verb;
{
	...
	if (obj->otyp == BOULDER && boulder_hits_pool(obj, x, y, FALSE))
		...
		(t = t_at(x,y)) != 0 && t->tseen &&
			(t->ttyp == PIT || t->ttyp == SPIKED_PIT || t->ttyp == GIANT_CHASM
			 || t->ttyp == MANA_PIT || t->ttyp == ANOXIC_PIT || t->ttyp == ACID_PIT)) {
...
register const char *word;
{
	...
	if ( (obj->otyp == LOADSTONE || obj->otyp == MANASTONE || obj->otyp == SLEEPSTONE || obj->otyp == LOADBOULDER || obj->otyp == STARLIGHTSTONE || obj->otyp == STONE_OF_MAGIC_RESISTANCE || obj->otyp == LUCKSTONE || obj->otyp == HEALTHSTONE) && obj->cursed) {
...
register struct obj *obj;
{
	...
	/* In Soviet Russia, picking up and dropping things requires time because everything's supposed to be . --Amy */
...
int retry;
{
	...
	if (n > 0) {
		...
		if (cnt < otmp->quan) {
		    if (welded(otmp)) {
			;	/* don't split */
		    } else if ( (otmp->otyp == LOADSTONE || otmp->otyp == LUCKSTONE || otmp->otyp == HEALTHSTONE || otmp->otyp == MANASTONE || otmp->otyp == SLEEPSTONE || otmp->otyp == LOADBOULDER || otmp->otyp == STARLIGHTSTONE || otmp->otyp == STONE_OF_MAGIC_RESISTANCE || is_feminismstone(otmp) ) && otmp->cursed) {
...
dodown()
{
	...
	if (!stairs_down && !ladder_down) {
		...
		if (!(trap = t_at(u.ux,u.uy)) ||
			...
			if ((trap = t_at(u.ux,u.uy)) && (trap->ttyp == PIT || trap->ttyp == MANA_PIT || trap->ttyp == GIANT_CHASM || trap->ttyp == SPIKED_PIT || trap->ttyp == ANOXIC_PIT || trap->ttyp == ACID_PIT) && !u.utrap) {
...
rerollchaloc:
		...
		if ( (Role_if(PM_GANG_SCHOLAR) || FemtrapActiveAriane) && !rn2(5)) {
			...
			switch (rnd(47)) {
				case 1:
				case 2: monstercolor = 38; break;
				case 3:
				case 4: monstercolor = 39; break;
				case 5:
				case 6: monstercolor = 40; break;
