...
mon_tele:
	...
	case MUSE_SCR_ROOT_PASSWORD_DETECTION:
		...
		if (otmp && otmp->oartifact == ART_ERASE_ALL_DATA && isevilvariant) {
			u.datadeletedefer = 1;
			datadeleteattack();
			verbalize("Ha ha ha ha ha ha ha, so you thought you could play in evil variant mode and somehow avoid all the evil variant? Well sucker, now your data has been deleted and you might as well give up.");
			if (rn2(2) || !ishaxor) m_useup(mtmp, otmp);	/* otmp might be free'ed */
			return 2;
		}
...
newboss:
	...
	case MUSE_SCR_HEALING:
		 ...
		 * But I mean, why the does the game just close, with no error message???
...
/*      case MUSE_WAN_TELEPORTATION:*/
	...
	case MUSE_SCR_FLOOD:
		...
		makeknown(otmp->otyp);
			...
			/* "scroll" (term created by Khor) - let's be nice, and reduce the chance of screwing up the player --Amy */
	...
	case MUSE_SCR_PAVING:
...

		{
			...
			if (madepoolSS)
				pline(FunnyHallu ?
						"Holy there are paved roads and large fountains and whoa you absolutely need to play part 2 of this game!" :
	...
	case MUSE_SCR_BARRHING:
		...
		makeknown(otmp->otyp);
			...
			if (madepoolF)
				pline(FunnyHallu ?
						"Aw, this feels like being in a jail!" :
