...
selecttrap2:
		...
		if (In_sokoban(&u.uz) && rn2(10) && (rtrap == HOLE || rtrap == TRAPDOOR || rtrap == SHAFT_TRAP || rtrap == CURRENT_SHAFT || rtrap == PIT || rtrap == SPIKED_PIT || rtrap == GIANT_CHASM || rtrap == MANA_PIT || rtrap == ANOXIC_PIT || rtrap == ACID_PIT)) goto selecttrap2;
...
struct mkroom	*croom;
{
	...
	if (!class)
	    ...
	    /* Amy edit: what the man why did this allow G_NOGEN monsters to spawn */
	}
	x = m->x;
	y = m->y;
	if (croom)
...
retryrandtype:
	...
	if (croom->rtype == RANDOMROOM) {
		...
		switch (rnd(84)) {
			...
			case 56: croom->rtype = MENAGERIE; break;
...
dlb *fd;
{
	...
	if (moves == 1 && !rn2(2)) { /* some earlygame help... --Amy */
	    ...
	    for(x = rn2((int) ((rn2(4) ? 15 : 30) * mapfact) / 100); x; x--) {
			...
			while (trytrap == PIT || trytrap == SPIKED_PIT || trytrap == GIANT_CHASM || trytrap == MANA_PIT || trytrap == ANOXIC_PIT || trytrap == ACID_PIT || trytrap == SHAFT_TRAP || trytrap == CURRENT_SHAFT ||
