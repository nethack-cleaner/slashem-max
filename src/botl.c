...
#endif
{
	...
	sprintf(nb = eos(nb), "%s", urole.filecode); /* fully disclosing what character you're playing */
	sprintf(nb = eos(nb), "%s",  urace.filecode); /* abbreviated so the line doesn't roll over --Amy */
...
*/
	...
	if (u.urealedibility && u.uhunger >= 4500) 
#if defined(STATUS_COLORS) && defined(TEXTCOLOR)
