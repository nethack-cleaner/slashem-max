...
menu_item **pick_list;	/* list of objects and counts to pick up */
{
	...
	for (n = 0, curr = olist; curr; curr = FOLLOW(curr, follow)) {
		...
		(flags.pickup_thrown && curr->was_thrown && !(curr->cursed && curr->bknown && !flags.pickup_cursed && !Hallucination ) ) )
#else
	    if ((/*!*otypes || */index(otypes, curr->oclass) ||
		(flags.pickup_thrown && curr->was_thrown && !(curr->cursed && curr->bknown && !flags.pickup_cursed && !Hallucination ) ) ||
	...
	for (curr = olist; curr; curr = FOLLOW(curr, qflags)) {
	  if ((*allow)(curr)) {
	    if ((iflags.sortloot == 'f' ||
	        (iflags.sortloot == 'l' && !(qflags & USE_INVLET))) && !Hallucination)
	      {
...
int how;			/* type of query */
{
	...
	*pick_list = (menu_item *) 0;
	...
	if ((qflags & BUC_BLESSED) && count_buc(olist, BUC_BLESSED) && !Hallucination) {
	    do_blessed = TRUE;
	    num_buc_types++;
	}
	if ((qflags & BUC_CURSED) && count_buc(olist, BUC_CURSED) && !Hallucination) {
	    do_cursed = TRUE;
	    num_buc_types++;
	}
	if ((qflags & BUC_UNCURSED) && count_buc(olist, BUC_UNCURSED) && !Hallucination) {
	    do_uncursed = TRUE;
	    num_buc_types++;
	}
	if ((qflags & BUC_UNKNOWN) && count_buc(olist, BUC_UNKNOWN) && !Hallucination) {
	    do_buc_unknown = TRUE;
	    num_buc_types++;
	}
...
#ifndef GOLDOBJ
    } else if (flags.knapsacklimit && obj->otyp != LOADSTONE && obj->otyp != HEALTHSTONE && obj->otyp != LUCKSTONE && obj->otyp != MANASTONE && obj->otyp != SLEEPSTONE && obj->otyp != LOADBOULDER && obj->otyp != STARLIGHTSTONE && obj->otyp != STONE_OF_MAGIC_RESISTANCE && !is_feminismstone(obj) && obj->oclass != COIN_CLASS && inv_cnt() >= 52 &&
		!merge_choice(invent, obj)) {
#else
    } else if (flags.knapsacklimit && obj->otyp != LOADSTONE && obj->otyp != HEALTHSTONE && obj->otyp != LUCKSTONE && obj->otyp != MANASTONE && obj->otyp != SLEEPSTONE && obj->otyp != LOADBOULDER && obj->otyp != STARLIGHTSTONE && obj->otyp != STONE_OF_MAGIC_RESISTANCE && !is_feminismstone(obj) && inv_cnt() >= 52 && !merge_choice(invent, obj)) {
#endif
...
boolean alwaysflag;	/* force the item to be picked up even if it burdens you --Amy */
{
	...
	if (obj && obj->oclass == POTION_CLASS && !rn2(3) && (Monsterfingers || u.uprops[MONSTERFINGERS_EFFECT].extrinsic) ) {
...
#endif
	if (obj->quan != count && obj->otyp != LOADSTONE && obj->otyp != LUCKSTONE && obj->otyp != HEALTHSTONE && obj->otyp != MANASTONE && obj->otyp != SLEEPSTONE && obj->otyp != LOADBOULDER && obj->otyp != STARLIGHTSTONE && obj->otyp != STONE_OF_MAGIC_RESISTANCE && !is_feminismstone(obj) )
...
boolean invobj;
{
	...
	if (!current_container) {
		...
		return 0;
	} else if ((obj->otyp == LOADSTONE || obj->otyp == LUCKSTONE || obj->otyp == HEALTHSTONE || obj->otyp == MANASTONE || obj->otyp == SLEEPSTONE || obj->otyp == LOADBOULDER || obj->otyp == STARLIGHTSTONE || obj->otyp == STONE_OF_MAGIC_RESISTANCE ) && obj->cursed) {
		obj->bknown = 1;
...
register struct obj *obj;
{
	...
	if (obj->quan != count && obj->otyp != LOADSTONE && obj->otyp != LUCKSTONE && obj->otyp != HEALTHSTONE && obj->otyp != MANASTONE && obj->otyp != SLEEPSTONE && obj->otyp != LOADBOULDER && obj->otyp != STARLIGHTSTONE && obj->otyp != STONE_OF_MAGIC_RESISTANCE )
	    obj = splitobj(obj, count);
