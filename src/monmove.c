...
/*	Pre-movement adjustments	*/
	...
	if (mdat->msound == MS_SHRIEK && u.usteed && (mtmp == u.usteed) && !um_dist(mtmp->mx, mtmp->my, 1))
	    m_respond(mtmp);

	if (mtmp->fartbonus > 9) mtmp->fartbonus = 9; /* fail save, gaaaaaah */

	if (FemtrapActiveMeltem && u.usteed && (mtmp == u.usteed) && mtmp->female && humanoid(mdat) && !rn2(10 + mtmp->butthurt - mtmp->fartbonus) && (distu(mtmp->mx, mtmp->my) <= fartdistance) && !mtmp->mpeaceful) {
	...
	if (mdat->msound == MS_PANTS && u.usteed && (mtmp == u.usteed) && !rn2(10) && !um_dist(mtmp->mx, mtmp->my, 1) && !mtmp->mpeaceful) {
	    m_respond(mtmp);
	}
	if (mdat->msound == MS_SOCKS && u.usteed && (mtmp == u.usteed) && !rn2(10) && !um_dist(mtmp->mx, mtmp->my, 1) && !mtmp->mpeaceful) {
	    m_respond(mtmp);
	}
	if ((mdat->msound == MS_ALLA || mtmp->egotype_alladrainer) && u.usteed && (mtmp == u.usteed) && !um_dist(mtmp->mx, mtmp->my, 1) && !mtmp->mpeaceful) {
		pline("alla");
		drain_alla(1);
	}
...
newbossSING:
	...
	if ( (mdat->msound == MS_SOUND || mtmp->egotype_sounder) && !rn2(20) && !um_dist(mtmp->mx, mtmp->my, 1) && !mtmp->mpeaceful)
	    m_respond(mtmp);
	if (mdat == &mons[PM_MEDUSA] && u.usteed && (mtmp == u.usteed) && couldsee(mtmp->mx, mtmp->my))
	    m_respond(mtmp);
	...
	if ((MonnoiseEffect || u.uprops[MONNOISE_EFFECT].extrinsic || have_monnoisestone()) && u.usteed && (mtmp == u.usteed) && !rn2(250) && !mtmp->mpeaceful && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) ) {
		switch (mdat->msound) {
			...
			case MS_SOCKS:
				...
				break;
			case MS_PANTS:
				if (canseemon(mtmp)) pline("Eww, %s stinks!", mon_nam(mtmp));
			...
			case MS_DOUGLAS_ADAMS:
				...
				break;
			case MS_PUPIL:
				verbalize("No, I do not have the homework today.");
				break;
			case MS_TEACHER:
	...
	if ((mdat->msound == MS_CONVERT || mtmp->egotype_converter) && u.usteed && (mtmp == u.usteed) && !Race_if(PM_TURMENE) && !Race_if(PM_HC_ALIEN) && !mtmp->mpeaceful && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !(uarmh && uarmh->oartifact == ART_JAMILA_S_BELIEF) && !rn2(10)) {
...
convertdone:
	...
	if ((mdat->msound == MS_HCALIEN || mtmp->egotype_wouwouer) && u.usteed && (mtmp == u.usteed) && !Race_if(PM_TURMENE) && !Race_if(PM_HC_ALIEN) && !mtmp->mpeaceful && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !(uarmh && uarmh->oartifact == ART_JAMILA_S_BELIEF) && !rn2(15)) {
	...
	if (mdat->msound == MS_SHOE && !mtmp->mpeaceful && evilfriday && u.usteed && (mtmp == u.usteed) && !rn2(50) && (distu(mtmp->mx, mtmp->my) <= 12)) {
	...
	if (spawnswithsneakers(mtmp->data) && !mtmp->mpeaceful && FemtrapActiveKerstin && u.usteed && (mtmp == u.usteed) && !rn2(50) && (distu(mtmp->mx, mtmp->my) <= 12)) {
		int paralysistime = mtmp->m_lev / 4;
		if (paralysistime > 1) paralysistime = rnd(paralysistime);
		if (paralysistime > 5) {
			while (rn2(5) && (paralysistime > 5)) {
	...
	if (mdat == &mons[PM_HUEPPOGREIFSCH] && multi >= 0 && u.usteed && (mtmp == u.usteed) && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !rn2(10)) {
		pline("Oh, damn hueppogreifsch...");
		nomul(-(rnd(5)), "unconscious from the hueppogreifsch", TRUE);
	}
	if (mdat == &mons[PM_WOK] && u.usteed && (mtmp == u.usteed) && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !rn2(10)) {
		verbalize("I am the WOK!");
	}

	if ((mdat == &mons[PM_NOISY_ANNOYANCE] || mdat == &mons[PM_DEMAGOGUE] || mdat == &mons[PM_UNDEAD_DEMAGOGUE]) && u.usteed && (mtmp == u.usteed) && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !rn2(20)) {
		demagogueparole();
		aggravate();
	}
	if (mdat == &mons[PM_GHOST_PORKER] && u.usteed && (mtmp == u.usteed) && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !rn2(20)) {
		verbalize("DENERF! DENERF! DENERF!");
	}
	if (mdat == &mons[PM_MIKRAANESIS] && u.usteed && (mtmp == u.usteed) && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !rn2(20)) {
	...
	if ((mdat->msound == MS_STENCH || mtmp->egotype_perfumespreader) && !Role_if(PM_HUSSY) && !(youmonst.data->msound == MS_STENCH) && u.usteed && (mtmp == u.usteed) && !mtmp->mpeaceful && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !rn2((mdat == &mons[PM_NICE_AUNTIE_HILDA]) ? 5 : (mdat == &mons[PM_AUNT_ANITA]) ? 5 : 20)) {
		switch (rnd(10)) {
	...
	if (mdat->msound == MS_BONES && u.usteed && (mtmp == u.usteed) && !mtmp->mpeaceful && (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && !rn2(100)) {
...
/*	Now, attack the player if possible - one attack set per monst	*/
	...
	/* Come on, monsters can still cuss at you if you can't see them. Seriously, what the heck. --Amy */
			...
			if (!strncmpi(plname, "Amy", 4) || !strncmpi(plalias, "Amy", 4) || !strncmpi(plname, "Bluescreen", 11) || !strncmpi(plalias, "Bluescreen", 11)) {
				...
				if (n > 0) {
					switch (selected[0].item.a_int) {
						case 1:
							verbalize("WHAT? Guys we got her, OVER HERE! Now we'll punish that demon!");
							break;
						case 2:
	    ...
	    if(inrange && (mtmp->data->msound == MS_STENCH || mtmp->data->msound == MS_SUPERMAN || mtmp->data->msound == MS_HCALIEN || mtmp->data->msound == MS_CONVERT || mtmp->data->msound == MS_FART_QUIET || mtmp->data->msound == MS_FART_NORMAL || mtmp->data->msound == MS_FART_LOUD) && (mtmp->data->geno & G_UNIQ) && !mtmp->mpeaceful && !rn2(10))
	    pline("%s %s", Monnam(mtmp), bosstaunt());
...
/* Boulder forts will be a lot less effective at holding dangerous monsters at bay. --Amy */
	...
	if (doorbuster) flag |= BUSTDOOR;
	{
		...
		if ((ptr == &mons[PM_TRACKER_JACKER] || (FemtrapActiveElla && mtmp->female && humanoid(mtmp->data) && (mtmp->mhp < (mtmp->mhpmax * 9 / 10) )) || ptr == &mons[PM_ASSHOLE_WHO_CANNOT_ENJOY_ANYTHING__NOT_EVEN_A_PAIR_OF_CUDDLE_HEELS] || ptr == &mons[PM_KILLER_GIANT_RAT] || ptr == &mons[PM_POLICE_DOG] || ptr == &mons[PM_POLICE_HUSKY] || ptr == &mons[PM_BIG_POLICE_DOG] || ptr == &mons[PM_CURSED____LEFTHANDED_FARTING_ELEPHANT] || ptr == &mons[PM_VERONA_MARBLE] || ptr == &mons[PM_CHASE_BIRD] || ptr == &mons[PM_JAYCEE] || ptr == &mons[PM_FULL_REFUGE] || ptr == &mons[PM_DRIVE_TRAIN] || ptr == &mons[PM_XTREME_TRACKER] || ptr == &mons[PM_REFUGE_UHLERT] || ptr == &mons[PM_THE_ULTIMATE_REFUGE]) && !mtmp->mpeaceful) appr = 1;
