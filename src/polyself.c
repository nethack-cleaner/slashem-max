...
controldone:
	 ...
	 */
	  ...
	  /* Amy edit: it's bullshit if you get newman() so often. If you polymorph, you should polymorph
...
int alone;
{
	...
	if (!Race_if(PM_TRANSFORMER) && (!alone || cantwield(youmonst.data))) {
	    ...
	    if ((!wep->cursed || (wep->otyp != LOADSTONE && wep->otyp != LUCKSTONE && wep->otyp != HEALTHSTONE && wep->otyp != MANASTONE && wep->otyp != SLEEPSTONE && wep->otyp != LOADBOULDER && wep->otyp != STARLIGHTSTONE && wep->otyp != STONE_OF_MAGIC_RESISTANCE ) ) && !(wep->otyp == LUCKSTONE && isevilvariant && !wep->cursed && !wep->blessed && Luck < 0))
		dropx(otmp);
...
weapondone:
	    ...
	    if (otmp2 != 0) {
		uswapwepgone();
		if ((!otmp2->cursed || (otmp2->otyp != LOADSTONE && otmp2->otyp != LUCKSTONE && otmp2->otyp != HEALTHSTONE && otmp2->otyp != MANASTONE && otmp2->otyp != SLEEPSTONE && otmp2->otyp != LOADBOULDER && otmp2->otyp != STARLIGHTSTONE && otmp2->otyp != STONE_OF_MAGIC_RESISTANCE ) ) && !(otmp2->otyp == LUCKSTONE && isevilvariant && !otmp2->cursed && !otmp2->blessed && Luck < 0))
		    dropx(otmp2);
	    }
	    untwoweapon();
