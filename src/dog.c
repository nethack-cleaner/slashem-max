...
makedog()
{
	...
	if (!Role_if(PM_SHOE_FETISHIST) && !Role_if(PM_JANITOR) && !Role_if(PM_YAUTJA) && !Role_if(PM_GRENADONIN) && !Role_if(PM_PSYKER) && !Role_if(PM_MILL_SWALLOWER) && !Role_if(PM_CHAOS_SORCEROR) && (preferred_pet == 'n')) return((struct monst *) 0);
...
maeney:
	...
	if (pettype == PM_NINJA_BOY) petname = "Cornelia"; /* My Immortal fanfic */
