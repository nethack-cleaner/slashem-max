...
int tmp;
{
	...
	/* Amy edit: the fact that they always did max damage was bad, IMHO. */
		...
		if (yn("Invoke the magical pentagram?") == 'y') {
			...
			if (rn2(3)) {
				...
				case 16:
					...
					int nastytrapdur = (Role_if(PM_GEEK) ? 12 : 24);
					if (!nastytrapdur) nastytrapdur = 24; /* fail safe */
					int blackngdur = (Role_if(PM_GEEK) ? 1000 : 500);
					...
					case 1:
						...
						if (intloss) {
							...
							if(!(HHallu_party & FROMOUTSIDE)) {
								You_feel( "a strange desire to celebrate.");
					...
					case 7:
						...
						if (intloss) {
							...
							if(!(HStoned_chiller & FROMOUTSIDE)) {
								You_feel("like relaxing on a couch.");
