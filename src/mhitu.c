			...
			if(( (!range2 || (Race_if(PM_WEAPON_XORN) && distu(mtmp->mx, mtmp->my) < 9) ) && (!MON_WEP(mtmp) || mtmp->mconf || Conflict ||
				...
				if ((tmp > (j = rnd(20+i))) || (uarmf && itemhasappearance(uarmf, APP_KOREAN_SANDALS) && !rn2(3) ) ) {
				    if ( (mattk->aatyp != AT_KICK || !rn2(5)) ||
					    (!thick_skinned(youmonst.data) && !(FemtrapActivePatricia) && !(uwep && uwep->oartifact == ART_ETRUSCIAN_SWIMMING_LESSON) && !Race_if(PM_DUTHOL) ) )
...
newboss:
	...
	if (Role_if(PM_FJORDE) && mtmp->data->mlet == S_EEL) {
		if(!range2 && (!MON_WEP(mtmp) || mtmp->mconf || Conflict || !touch_petrifies(youmonst.data))) {
			if (foundyou && !rn2(5) && tmp > (j = rnd(20+i))) {
				pline("%s nibbles you!", Monnam(mtmp));
				u.tremblingamount++;
			}
		}
	}
	...
	/* and now, the unholy satanic from hell, aka the most evil monster in existence... --Amy
...
dopois:
 ...
 do_stone:
		...
		if (is_animal(mtmp->data)) {
			...
			if (mtmp->mcan) break;
			/* Continue below */
		} else if (rn2(5) && !(u.uprops[ITEM_STEALING_EFFECT].extrinsic || ItemStealingEffect || (uarmc && uarmc->oartifact == ART_PERCENTIOEOEPSPERCENTD_THI) || (uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_COPPERED_OFF_FROM_ME) || have_stealerstone() ) && (dmgtype(youmonst.data, AD_SEDU) || (uarmg && uarmg->oartifact == ART_LORSKEL_S_SPECIAL_PROTECTI) || (uwep && uwep->oartifact == ART_ONE_HUNDRED_STARS)
		...
		if (!rn2(3) && atttyp != AD_SEDU && !issoviet && !(u.uprops[ITEM_STEALING_EFFECT].extrinsic || ItemStealingEffect || (uarmc && uarmc->oartifact == ART_PERCENTIOEOEPSPERCENTD_THI) || have_stealerstone() || (uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_COPPERED_OFF_FROM_ME) ) ) {
		...
		if (u.uprops[ITEM_STEALING_EFFECT].extrinsic || ItemStealingEffect || (uarmc && uarmc->oartifact == ART_PERCENTIOEOEPSPERCENTD_THI) || (uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_COPPERED_OFF_FROM_ME) || have_stealerstone() ) {
...
/* male characters are more susceptible to nymphs --Amy */
		...
		if(could_seduceX(mtmp, &youmonst, mattk) == 1
			&& !mtmp->mcan && (issoviet || iserosator || rn2(2) || (uarmf && itemhasappearance(uarmf, APP_LOLITA_BOOTS)) ) ) /* 50% chance --Amy */
		    if (doseduce(mtmp))
			return 3;
		break;
	    case AD_SAMU:
	...
	register struct attack  *mattk;
{
	...
	if (!u.uswallow) {	/* swallows you */
		...
		if ((t && ((t->ttyp == PIT) || (t->ttyp == SPIKED_PIT) || (t->ttyp == GIANT_CHASM) || (t->ttyp == MANA_PIT) || (t->ttyp == ANOXIC_PIT) || (t->ttyp == ACID_PIT) )) &&
	...
	switch(atttypA) {
		...
		case AD_FEMI:
			...
			pline("It feels very nice in here!");
...
do_stone2:
		...
		if (!rn2(3) && atttypA != AD_SEDU && !issoviet && !(u.uprops[ITEM_STEALING_EFFECT].extrinsic || ItemStealingEffect || (uarmc && uarmc->oartifact == ART_PERCENTIOEOEPSPERCENTD_THI) || have_stealerstone() || (uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_COPPERED_OFF_FROM_ME) ) ) {
			You_feel("a tug on your knapsack"); break;
		}
	...
	register struct attack  *mattk;
{
	...
	switch(atttypB) {
	    ...
	    case AD_SEDU:
	    case AD_SSEX:
		if (!rn2(3) && !issoviet && !(u.uprops[ITEM_STEALING_EFFECT].extrinsic || ItemStealingEffect || (uarmc && uarmc->oartifact == ART_PERCENTIOEOEPSPERCENTD_THI) || have_stealerstone() || (uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_COPPERED_OFF_FROM_ME) ) && canseemon(mtmp) && mtmp->mcansee ) break; /* no message, we don't want too much spam --Amy */

		if ((u.uprops[ITEM_STEALING_EFFECT].extrinsic || ItemStealingEffect || (uarmc && uarmc->oartifact == ART_PERCENTIOEOEPSPERCENTD_THI) || have_stealerstone() || (uwep && uwep->oartifact == ART_COPPERED_OFF_FROM_ME) || (u.twoweap && uswapwep && uswapwep->oartifact == ART_COPPERED_OFF_FROM_ME) ) && !mtmp->mcan && canseemon(mtmp) && mtmp->mcansee) {
     ...
     break;
	    ...
	    case AD_FREN:
	        if(!mtmp->mcan && canseemon(mtmp) && mtmp->mcansee && (issoviet || !rn2(6))) {
	                pline("%s shouts", Monnam(mtmp), flags.female ? "nonexistant" : "little");
		    stop_occupation();
...
#ifdef SHOW_DMG
	if (flags.showdmg && !(DamageMeterBug || u.uprops[DAMAGE_METER_BUG].extrinsic || have_damagemeterstone())) {
...
register struct monst *mtmp;
{
	...
	if (yn("Do you want to clean them?") == 'y') {
			...
			else set_occupation(singcleaning, "cleaning", 0);
			mtmp->mpeaceful = TRUE;
			mtmp->singannoyance = FALSE;
			pline("You start cleaning", Monnam(mtmp));
