...
int traitno;
{
	...
	switch (traitno) {
		...
		case 174:
			return "monsters that have attacks";
		case 175:
		...
		case 226:
			return "monsters whose attacks start encounters";
		case 227:
...
domonability()
{
	...
	/* snail can't equip pick-axes, so should be able to dig without one from time to time --Amy */
	...
	} else if (uarmf && uarmf->oartifact == ART_ENDLESS_DESEAMING && yn("Do you want to clean your shoes?")=='y') {
		if (!uarmf->oeroded && !uarmf->oeroded2) {
			pline("Your shoes currently don't need cleaning.");
			return TRUE;
		} else {
			delay = -(rn1(50, 50));
	...
	} else if (uarmf && uarmf->oartifact == ART_THAT_S_SUPER_UNFAIR && yn("Do you want to clean your shoes?")=='y') {
		if (!uarmf->oeroded && !uarmf->oeroded2) {
			pline("Your shoes currently don't need cleaning.");
			return TRUE;
		} else {
		...
		for (i = -bd; i <= bd; i++) for(j = -bd; j <= bd; j++) {
			...
			if ((ttmp = t_at(u.ux + i, u.uy + j)) != 0) {
				if (ttmp->ttyp == MAGIC_PORTAL || ttmp->ttyp == HOLE || ttmp->ttyp == TRAPDOOR || ttmp->ttyp == SHAFT_TRAP || ttmp->ttyp == CURRENT_SHAFT || ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT || ttmp->ttyp == GIANT_CHASM || ttmp->ttyp == MANA_PIT || ttmp->ttyp == ANOXIC_PIT || ttmp->ttyp == ACID_PIT) {
					undtrpcnt++;
					continue;
				}
				deltrap(ttmp);
...
boolean guaranteed;
{
	...
	if ((guaranteed || !rn2(10)) && (Monsterfingers || u.uprops[MONSTERFINGERS_EFFECT].extrinsic)) {
	...
	if ((guaranteed || !rn2(10)) && (u.uprops[CRAP_EFFECT].extrinsic || CrapEffect )) {
	...
	if ((guaranteed || !rn2(10)) && (Fast && !(uarmf && uarmf->oartifact == ART_UNEVEN_ENGINE) && !(uarmf && uarmf->oartifact == ART_ERROR_IN_PLAY_ENCHANTMENT))) you_are(StrongFast ? "ultra fast" : Very_fast ? "very fast" : "fast");
	if ((guaranteed || !rn2(10)) && (!Fast && Very_fast && !(uarmf && uarmf->oartifact == ART_UNEVEN_ENGINE) && !(uarmf && uarmf->oartifact == ART_ERROR_IN_PLAY_ENCHANTMENT))) you_are("very fast");
...
int final;
{
	...
	if (Monsterfingers || u.uprops[MONSTERFINGERS_EFFECT].extrinsic) {
		sprintf(buf, "the following problem: Picking up potions may break them and subject you to their vapors.");
	...
	if (u.uprops[CRAP_EFFECT].extrinsic || CrapEffect ) {
		sprintf(buf, "diarrhea");
	...
	if (Fast && !(uarmf && uarmf->oartifact == ART_UNEVEN_ENGINE) && !(uarmf && uarmf->oartifact == ART_ERROR_IN_PLAY_ENCHANTMENT)) dump(youwere, StrongFast ? "ultra fast" : Very_fast ? "very fast" : "fast");
	if (!Fast && Very_fast && !(uarmf && uarmf->oartifact == ART_UNEVEN_ENGINE) && !(uarmf && uarmf->oartifact == ART_ERROR_IN_PLAY_ENCHANTMENT)) dump(youwere, "very fast");
...
minimal_enlightenment()
{
	...
	*xtrabuf = '\0';
	...
	if (hybridcount >= 20) sprintf(eos(xtrabuf), "(%d hybrids) ", hybridcount);
		...
		if (u.urealedibility && u.uhunger >= 4500) sprintf(eos(statline), "oversatiated, ");
		else if(strcmp(hux_stat[u.uhs], "        ")) sprintf(eos(statline), "%s, ", hux_stat[u.uhs]);
...
int final;
{
	...
	if (u.cnd_forcebuttcount) {
		...
		enl_msg(You_, "have bashed hostile ", "bashed hostile ", buf);
	}
...
register char *cmd;
{
	...
	if (*cmd && (AutopilotEffect || u.uprops[AUTOPILOT_EFFECT].extrinsic || have_autopilotstone() || Race_if(PM_RELEASIER) || (uarmf && uarmf->oartifact == ART_PRADA_S_DEVIL_WEAR) ) ) {
