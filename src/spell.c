...
age_spells()
{
	...
	for (i = 0; i < MAXSPELL && spellid(i) != NO_SPELL; i++)
		...
		if (!issoviet && !SpellColorCyan && !(SpellForgetting || u.uprops[SPELL_FORGETTING].extrinsic || have_spellforgettingstone()) && spellknow(i) == 1000) pline("Your %s spell is beginning to fade from your memory.", spellname(i));
		if (!issoviet && !SpellColorCyan && !(SpellForgetting || u.uprops[SPELL_FORGETTING].extrinsic || have_spellforgettingstone()) && spellknow(i) == 100) pline("You are about to forget the %s spell.", spellname(i));
		if (!issoviet && !SpellColorCyan && !(SpellForgetting || u.uprops[SPELL_FORGETTING].extrinsic || have_spellforgettingstone()) && spellknow(i) == 0) pline("You no longer know how to cast the %s spell.", spellname(i));
...
magicalenergychoice:
			if (!getdir((char *)0)) {
				...
				/* Amy edit: this is absolute bad behavior, because it's very easy to mistype. It should
	...
	case SPE_ELEMENTAL_MINION:
		{
			...
			/* make good stuff, after all you pay with max health for this spell */
			u.aggravation = 1;
	...
	case SPE_HYPERSPACE_SUMMON:
		{
			...
			/* make good stuff, after all you pay with max health and pw for this spell */
			u.aggravation = 1;
...
rerollX:
	...
	/* charging is way too overpowered, let's add another "downside" --Amy */
...
int spell;
{
	...
	/* make it quite a bit harder to get to 0% fail, because spells are generally easier to
