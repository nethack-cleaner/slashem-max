...
draw_horizontal(int x, int y, int hp, int hpmax)
{
	...
	wprintw(win, " ");
	wprintw(win, urole.filecode);
	wprintw(win, urace.filecode);
	wprintw(win, flags.female ? "Fem" : "Mal");
	wprintw(win, (u.ualign.type == A_CHAOTIC) ? "Cha" : (u.ualign.type == A_NEUTRAL) ? "Neu" : "Law");
